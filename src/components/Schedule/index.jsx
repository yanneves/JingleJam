...
import {
  CreatorDetails,
  CreatorThumbnail,
} from "../DetailStreamer/StreamerDetails"
    ...
    return (
      <div className="home">
        <div
          className="donate"
          onClick={() =>
            window.open("https://humblebundle.com/yogscast", "_blank")
          }
        >
          donate here!
        </div>
        <header>
          ...
          <h1>
            Stream Schedule <span />{" "}
            <div>
              {this.state.streams.meta.week}
              <sup>
                {this.state.streams.meta.week === 1
                  ? "st"
                  : this.state.streams.meta.week === 2
                  ? "nd"
                  : this.state.streams.meta.week === 3
                  ? "rd"
                  : "th"}
              </sup>{" "}
              week
        ...
        <div className="controls">
          {this.state.week === 1 ? null : streams.weeks[this.state.week - 2]
              .meta.active ? (
            <article className="prev" onClick={this.prevWeek}>
              <FiChevronLeft />
            </article>
          ) : null}
          {this.state.week === 5 ? null : streams.weeks[this.state.week].meta
              .active ? (
            <article className="next" onClick={this.nextWeek}>
              <FiChevronRight />
            </article>
          ) : null}
        </div>
...
class Schedule extends Component {
  ...
  initLiveTime() {
    ...
    if (date.getUTCHours() < 14) {
      livePosition = `calc(55px + 10px + (((((100% - 55px) / 4) - 10px) / 180) * ${(date.getUTCHours() -
        11) *
        60 +
        date.getUTCMinutes()}))`
    } else if (date.getUTCHours() < 17) {
      livePosition = `calc(55px + 10px + ((((100% - 55px) / 4) - 10px) * 1) (((((100% - 55px) / 4) - 10px) / 180) * ${(date.getUTCHours() -
        11) *
        60 +
        date.getUTCMinutes()}))`
    } else if (date.getUTCHours() < 20) {
      livePosition = `calc(55px + 10px + ((((100% - 55px) / 4) - 10px) * 2) (((((100% - 55px) / 4) - 10px) / 180) * ${(date.getUTCHours() -
        11) *
        60 +
        date.getUTCMinutes()}))`
    } else if (date.getUTCHours() < 23) {
      livePosition = `calc(55px + 10px + ((((100% - 55px) / 4) - 10px) * 3) (((((100% - 55px) / 4) - 10px) / 180) * ${(date.getUTCHours() -
        11) *
        60 +
        date.getUTCMinutes()}))`
    }
          ...
          ? this.state.times.map((time, i) => {
              return (
                <article className={`time _${time.start}`} key={i}>
                  <h3>
                    {time.startOffset < 10
                      ? `0${time.startOffset}:00`
                      : `${time.startOffset}:00`}
                  </h3>
                  <h1>{time.name}</h1>
                  <h4>
                    {time.finishOffset < 10
                      ? `0${time.finishOffset}:00`
                      : `${time.finishOffset}:00`}
                  </h4>
                </article>
              )
            })
                ...
                <article className={`day _${date % 7}`} key={i}>
                  {
                    [
                      ...
                      "friday",
                      "saturday",
                    ][new Date(Date.UTC(2019, 11, date, 11, 0, 0, 0)).getDay()]
                  }
              ...
              ? this.state.streams.days[0].streams.map((stream, i) => {
                  return (
                    <StreamRouter
                      ...
                      date={this.state.streams.days[0].date}
                      pretitle={stream.pretitle}
                      title={stream.title}
                      subtitle={stream.subtitle}
                      subtitle2={stream.subtitle2}
                      startHour={stream.startHour}
                      starring={stream.starring}
              ...
              ? this.state.streams.days[1].streams.map((stream, i) => {
                  return (
                    <StreamRouter
                      key={i}
                      week={this.state.week}
                      date={this.state.streams.days[1].date}
                      pretitle={stream.pretitle}
                      title={stream.title}
                      subtitle={stream.subtitle}
                      subtitle2={stream.subtitle2}
              ...
              ? this.state.streams.days[2].streams.map((stream, i) => {
                  return (
                    <StreamRouter
                      key={i}
                      week={this.state.week}
                      date={this.state.streams.days[2].date}
                      pretitle={stream.pretitle}
                      title={stream.title}
                      subtitle={stream.subtitle}
                      subtitle2={stream.subtitle2}
              ...
              ? this.state.streams.days[3].streams.map((stream, i) => {
                  return (
                    <StreamRouter
                      key={i}
                      week={this.state.week}
                      date={this.state.streams.days[3].date}
                      pretitle={stream.pretitle}
                      title={stream.title}
                      subtitle={stream.subtitle}
                      subtitle2={stream.subtitle2}
              ...
              ? this.state.streams.days[4].streams.map((stream, i) => {
                  return (
                    <StreamRouter
                      key={i}
                      week={this.state.week}
                      date={this.state.streams.days[4].date}
                      pretitle={stream.pretitle}
                      title={stream.title}
                      subtitle={stream.subtitle}
                      subtitle2={stream.subtitle2}
              ...
              ? this.state.streams.days[5].streams.map((stream, i) => {
                  return (
                    <StreamRouter
                      key={i}
                      week={this.state.week}
                      date={this.state.streams.days[5].date}
                      pretitle={stream.pretitle}
                      title={stream.title}
                      subtitle={stream.subtitle}
                      subtitle2={stream.subtitle2}
              ...
              ? this.state.streams.days[6].streams.map((stream, i) => {
                  return (
                    <StreamRouter
                      key={i}
                      week={this.state.week}
                      date={this.state.streams.days[6].date}
                      pretitle={stream.pretitle}
                      title={stream.title}
                      subtitle={stream.subtitle}
                      subtitle2={stream.subtitle2}
...
class Stream extends Component {
  ...
  expandStream() {
    this.props.history.push(
      `/stream/${this.props.week}.${this.props.date}.${this.props.startHour}`
    )
  }
  ...
  render() {
    return (
      <article
        onClick={this.expandStream}
        className={`stream _${this.props.startHour} _${this.props.date %
          7} duration_${this.props.duration} ${this.props.style}`}
      >
        {this.props.pretitle !== undefined ? (
          <h3>{this.props.pretitle}</h3>
        ) : null}
...
class ExpandedStream extends Component {
  ...
  componentDidMount() {
    ...
    const { days } = streams.weeks[parseInt(id[0]) - 1]
    ...
    this.setState(
      { id, stream, vod, idString: this.props.match.params.streamid },
      this.updateState
    )
  }
  ...
  updateState() {
    ...
    const start = new Date(
      Date.UTC(2019, 11, this.state.id[1], this.state.stream.startHour, 0, 0, 0)
    )
    const end = new Date(
      Date.UTC(
        2019,
        11,
        this.state.id[1],
        this.state.stream.startHour + 3 * this.state.stream.duration,
        0,
        0,
        0
      )
  ...
  openProfile(creator) {
    this.setState({ profile: true, creator })
  }
        ...
        <article className={`content ${this.state.stream.style}`}>
          ...
          {!this.state.profile ? (
            <div className="details">
              {this.state.stream.title !== undefined ? (
                <h1>{this.state.stream.title}</h1>
              ) : null}
              {this.state.stream.pretitle !== undefined ? (
                <h3>{this.state.stream.pretitle}</h3>
              ) : null}
              {this.state.stream.subtitle !== undefined ? (
                <h4>{this.state.stream.subtitle}</h4>
              ) : null}
              {this.state.stream.subtitle2 !== undefined ? (
                <h5>{this.state.stream.subtitle2}</h5>
              ) : null}
              {this.state.state === "ended" ? (
                this.props.watched.indexOf(this.state.idString) === -1 ? (
                  <div
                    className="watched"
                    onClick={() => this.props.addWatched(this.state.idString)}
                  >
                    <FiCheck />
                  </div>
                ) : (
                  <div
                    className="watched filled"
                    onClick={() =>
                      this.props.removeWatched(this.state.idString)
                    }
                  >
                    <FiCheck />
                  </div>
              ...
              <div className="starring">
                {this.state.stream.starring === undefined
                  ? null
                  : this.state.stream.starring.map((creator, i) => {
                      return (
                        <CreatorThumbnail
                          key={i}
            ...
            <CreatorDetails
              ...
              creator={creators[this.state.creator]}
            ...
            {this.state.state === "upcoming" ? (
              this.state.countdown === null ? null : (
                <div className="countdown">
                  ...
                  <h1>
                    ...
                    :
                    <span data-tooltip="hours">
                      {this.state.countdown.hours}
                    </span>
                    :
                    <span data-tooltip="minutes">
                      {this.state.countdown.minutes}
                    </span>
                    :
                    <span data-tooltip="seconds">
              ...
              <div className="live">
                <div className="iframe_container">
                  <iframe
                    title="YOGSCAST Twitch Stream"
                    src="https://player.twitch.tv/?channel=yogscast"
                    frameBorder="0"
                    allowFullScreen
                    scrolling="no"
                    height="100%"
              ...
              this.state.stream.vods.twitch === undefined &&
              this.state.stream.vods.youtube === undefined ? (
                <h1 className="novods">
                  there are currently no vods available for this stream
                </h1>
              ) : (
                <div className="allvods">
                  <div className="iframe_container">
                    {this.state.vod === "twitch" ? (
                      <iframe
                        title={`${this.state.stream.title} Twitch VOD`}
                        src={`https://player.twitch.tv/?autoplay=false&t=${this.state.stream.vods.twitch.time}&video=v${this.state.stream.vods.twitch.id}`}
                        frameBorder="0"
                        allowFullScreen
                        scrolling="no"
                        height="100%"
                      ...
                      <iframe
                        title={`${this.state.stream.title} YouTube VOD`}
                        src={`https://youtube.com/embed/${this.state.stream.vods.youtube.id}`}
                        frameBorder="0"
                        allowFullScreen
                        width="100%"
                        height="100%"
                  ...
                  <ul className="toggle">
                    {this.state.stream.vods.twitch !== undefined ? (
                      <li
                        className={`twitch ${
                          this.state.vod === "twitch" ? "active" : null
                        }`}
                        onClick={() => this.setState({ vod: "twitch" })}
                      >
                        catch up on <FaTwitch />
                      </li>
                    ) : null}
                    {this.state.stream.vods.youtube !== undefined ? (
                      <li
                        className={`youtube ${
                          this.state.vod === "youtube" ? "active" : null
                        }`}
                        onClick={() => this.setState({ vod: "youtube" })}
                      >
                        catch up on <FaYoutube />
                      </li>
                    ) : null}
                  </ul>
                  <ul className="mobile">
                    {this.state.stream.vods.twitch !== undefined ? (
                      <a
                        href={`https://twitch.tv/videos/${this.state.stream.vods.twitch.id}?t=${this.state.stream.vods.twitch.time}`}
                      >
                        <li className="twitch">
                          catch up on <FaTwitch />
                        </li>
                    ...
                    {this.state.stream.vods.youtube !== undefined ? (
                      <a
                        href={`https://youtube.com/watch?v=${this.state.stream.vods.youtube.id}`}
                      >
